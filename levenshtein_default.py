...
def query_solr(endpoint, query, start=0, rows=200, sort="date desc", fl="bibcode"):
    ...
    if response.status_code == 200:
        ...
        return results
    logger.warn(
        "For query {0}, there was a network problem: {1}\n".format(query, response)
    )
    return None
...
def plot_hist(orcid_field="orcid_user", save_path=os.getcwd()):
    ...
    n, bins, patches = py.hist(
        lev, bin_edges, facecolor=plcol, alpha=0.75, edgecolor="black"
    )

    nperc = 100.0 * n / n.sum()
    for i in range(len(bins[0:-1])):
        if nperc[i] > 0.1:
            py.text(bins[i], n[i] + n.max() * 0.01, "{:.1f}%".format(nperc[i]))
